...
```bash
# 파일 선택 및 업로드 UI
/sc:implement "콘텐츠 업로드 화면 - 파일 선택, 썸네일, 메타데이터" --persona-frontend --c7 --seq

# 업로드 진행 표시
/sc:implement "업로드 진행률 표시 - 프로그레스 바, 시간 예상" --persona-frontend --c7 --validate

# 미리보기 기능
/sc:implement "콘텐츠 미리보기 - 이미지/비디오 프리뷰, 편집 도구" --persona-frontend --c7 --seq

# 업로드 시뮬레이션 로직
/sc:implement "업로드 시뮬레이션 - Mock 업로드, 성공/실패 처리" --persona-backend --validate
```
