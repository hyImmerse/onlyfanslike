...
### 2025-01-08 (오후)
- [x] UI 개선 작업 완료
  - 구글 로그인 버튼에 컬러풀한 구글 로고 적용
    - CustomPainter를 사용한 구글 로고 직접 렌더링
    - 4색 구글 브랜드 컬러 정확히 구현
  - 메인 화면 프로필 아바타 다크 테마 대응
    - 보라색(#7C4DFF) 배경에 흰색 아이콘
    - 다크/라이트 테마 모두에서 가시성 확보
  - 채팅 및 대화 목록 화면 프로필 이미지 개선
    - Unsplash API 연동으로 실제 인물 사진 10개 제공
    - 사용자/대화별 일관된 프로필 이미지 (해시 기반)
    - 채팅 화면, 메시지 버블, 대화 목록 모두 적용
